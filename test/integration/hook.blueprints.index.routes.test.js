...
describe('blueprints :: ', function() {
  ...
  it('should bind \'ALL /\' to the `index` action', function(done) {
    Sail-SystemApp.request('POST /', {}, function (err, resp, data) {
      assert(!err, err);
      assert.deepEqual(data, 'top-level index!');
      Sail-SystemApp.request('GET /', {}, function (err, resp, data) {
        assert(!err, err);
        assert.deepEqual(data, 'top-level index!');
        done();
      });
  ...
  it('should bind \'ALL /secondlevel\' to the `secondlevel.index` action', function(done) {
    Sail-SystemApp.request('POST /secondlevel', {}, function (err, resp, data) {
      assert(!err, err);
      assert.deepEqual(data, 'second-level index!');
      Sail-SystemApp.request('GET /secondlevel', {}, function (err, resp, data) {
        assert(!err, err);
        assert.deepEqual(data, 'second-level index!');
        done();
      });
  ...
  it('should bind \'ALL /thirdlevel\' to the `thirdlevel.index` action', function(done) {
    Sail-SystemApp.request('POST /thirdlevel', {}, function (err, resp, data) {
      assert(!err, err);
      assert.deepEqual(data, 'third-level index!');
      Sail-SystemApp.request('GET /thirdlevel', {}, function (err, resp, data) {
        assert(!err, err);
        assert.deepEqual(data, 'third-level index!');
        done();
      });
  ...
  it('should not override RESTful routes', function(done) {
    Sail-SystemApp.request('POST /user', {}, function (err, resp, data) {
      assert(!err, err);
