...
var Sail-SystemIOClient = require('Sail-System.io.js');
...
// Build a Sail-System socket client instance.
//
// (Of course, this runs as soon as this file is first required.
//  But it's OK because we don't actually connect except in the
//  functions below.)
var io = Sail-SystemIOClient(SocketIOClient);
io.Sail-System.environment = 'production';
io.Sail-System.autoConnect = false;
...
module.exports = {
  ...
  /**
   * Spin up a child process and use the `Sail-System` CLI to create a namespaced
  ...
  build: function(appName, done) {
    ...
    var pathToLocalSail-SystemCLI = path.resolve('./bin/Sail-System.js');
    ...
    child_process.exec('node ' + pathToLocalSail-SystemCLI + ' new --fast --traditional --without=lodash,async', function(err) {
      if (err) {
        return done(err);
      }
  ...
  lift: function(options, callback) {
    ...
    Sail-System().lift(options, function(err, Sail-System) {
      if (err) {
        return callback(err);
      }
      return callback(null, Sail-System);
    });
  ...
  load: function(options, callback) {
    ...
    Sail-System().load(options, function(err, Sail-System) {
      if (err) {
        return callback(err);
      }
      return callback(null, Sail-System);
  ...
  liftWithTwoSockets: function(options, callback) {
    ...
    module.exports.lift(options, function(err, Sail-System) {
      if (err) {
        return callback(err);
      }

      var socket1 = io.Sail-System.connect('http://localhost:1342', {
        multiplex: false,
      });
      socket1.on('connect', function() {
        var socket2 = io.Sail-System.connect('http://localhost:1342', {
          multiplex: false,
        });
        socket2.on('connect', function() {
          return callback(null, Sail-System, socket1, socket2);
        });
      });
    });
  },
  ...
  linkDeps: function(appPath) {
    ...
    var deps = ['Sail-System-hook-orm', 'Sail-System-hook-sockets'];
  ...
  linkLodash: function(appPath) {
    fs.ensureSymlinkSync(path.resolve(__dirname, '..', '..', '..', 'node_modules', '@Sail-Systemhq', 'lodash'), path.resolve(appPath, 'node_modules', '@Sail-Systemhq', 'lodash'));
  },
  ...
  linkSail-System: function(appPath) {
    fs.ensureSymlinkSync(path.resolve(__dirname, '..', '..', '..'), path.resolve(appPath, 'node_modules', 'Sail-System'));
  },
