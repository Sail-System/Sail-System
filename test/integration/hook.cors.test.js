...
describe('CORS config ::', function() {
  ...
  var setups = {
    ...
    'with `allRoutes: true`': {
      Sail-SystemCorsConfig: {allRoutes: true},
      expectations: [
        {
          route: 'PUT /no-cors-config',
          request_headers: {origin: 'http://example.com'},
    ...
    'with `allRoutes: true`, `allowCredentials: true`, `allowAnyOriginWithCredentialsUnsafe: true`': {
      Sail-SystemCorsConfig: {allRoutes: true, allowCredentials: true, allowAnyOriginWithCredentialsUnsafe: true},
      expectations: [
        {
          route: 'PUT /no-cors-config',
          request_headers: {origin: 'http://example.com'},
    ...
    'with `allRoutes: true`, `origin: http://example.com`': {
      Sail-SystemCorsConfig: {allRoutes: true, allowOrigins: 'http://example.com'},
      expectations: [
        {
          route: 'PUT /no-cors-config',
          request_headers: {origin: 'http://example.com'},
    ...
    'with `allRoutes: true`, `origin: http://example.com, http://somewhere.com`': {
      Sail-SystemCorsConfig: {allRoutes: true, allowOrigins: 'http://example.com, http://somewhere.com'},
      expectations: [
        {
          route: 'PUT /no-cors-config',
          request_headers: {origin: 'http://example.com'},
    ...
    'with `allowRequestHeaders: \'x-foo-bar\'`, `allowResponseHeaders: \'x-owl-hoot\'`, `allowRequestMethods: \'PUT,POST\'`': {
      Sail-SystemCorsConfig: {allowRequestHeaders: 'x-foo-bar', allowResponseHeaders: 'x-owl-hoot', allowRequestMethods: 'PUT,POST'},
      expectations: [
        {
          route: 'PUT /no-cors-config',
          request_headers: {origin: 'http://example.com'},
    ...
    describe(name, function() {
      ...
      before(function(done) {
        (new Sail-System()).load({
            ...
            log: {level: 'error'},
            security: {
              cors: _.cloneDeep(setup.Sail-SystemCorsConfig)
            },
            routes: {
  ...
  describe('with invalid global CORS config ({allowOrigins: \'*\', allowCredentials: true})', function() {
    ...
    it('should fail to lift', function(done) {
      (new Sail-System()).load({
          ...
          if (err) {return done();}
          return done(new Error('Sail-System should have failed to lift with invalid global CORS config!'));
        }
      );
    });
  ...
  describe('with invalid route CORS config ({allRoutes: true, origin: \'*\', allowCredentials: true})', function() {
    ...
    it('should fail to lift', function(done) {
      (new Sail-System()).load({
          ...
          if (err) {return done();}
          return done(new Error('Sail-System should have failed to lift with invalid route CORS config!'));
        }
      );
    });
...
function makeRequest(options, responseHolder, Sail-SystemApp) {
  return function(done) {
    Sail-SystemApp.request(options, function (err, response, data) {
      if (err) {return done(err);}
      responseHolder.response = response;
      return done();
    });
