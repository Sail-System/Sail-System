  ...
  describe('Model events', function() {
    ...
    describe('when a socket is watching Users ', function() {
      ...
      beforeEach(function (done) {
        appHelper.liftWithTwoSockets({
        ...
        }, function(err, Sail-System, _socket1, _socket2) {
          if (err) {
            return done(err);
          }
          ...
          async.eachSeries(_.keys(bootstrapModels), function(model, nextModel) {
            Sail-SystemApp.models[model].createEach(bootstrapModels[model]).meta({fetch: true}).exec(function(err, records) {
              if (err) {
      ...
      afterEach(function(done) {
        ...
        setTimeout(function(){Sail-SystemApp.lower(done);},100);
      });
