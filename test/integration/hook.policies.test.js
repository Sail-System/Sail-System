...
describe('policies :: ', function() {
  ...
  describe('basic usage :: ', function() {
    ...
    it('should load policies from disk and merge them with programmatically added policies', function(done) {
      ...
      }, function(err, Sail-SystemApp) {
        if (err) { return done(err); }
        assert.equal(_.keys(Sail-SystemApp.hooks.policies.middleware).length, 2);
        assert(Sail-SystemApp.hooks.policies.middleware.foo);
        assert(Sail-SystemApp.hooks.policies.middleware.err);
        return done();
      });
    });
  ...
  describe('with invalid configuration :: ', function() {
    ...
    describe('when a non-function policy is specified on disk', function() {
      ...
      it('Sail-System should fail to lift', function(done) {
