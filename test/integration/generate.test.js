describe('API and adapter generators', function() {
  ...
  before(function(done) {
    ...
    exec('node ' + Sail-SystemBin + ' new ' + appName + ' --fast --traditional --without=lodash,async', function(err) {
      ...
      // Move into app directory and update Sail-SystemBin relative path
  ...
  describe('Sail-System generate model <modelname>', function() {
    var modelName = 'user';

    it('should throw an error if no model name is specified', function(done) {

      exec('node ' + Sail-SystemBin + ' generate model', function(err) {
    ...
    it('should create a model file in models folder', function(done) {
      ...
      exec('node ' + Sail-SystemBin + ' generate model ' + modelName, function(err) {
  ...
  describe('Sail-System generate controller <controllerName>', function() {
    var controllerName = 'user';

    it('should throw an error if no controller name is specified', function(done) {

      exec('node ' + Sail-SystemBin + ' generate controller', function(err) {
        assert.equal(err.code, 1);
        done();
      });
    });
    ...
    it('should create a controller file in controllers folder', function(done) {
      ...
      exec('node ' + Sail-SystemBin + ' generate controller ' + controllerName, function(err) {
        if (err) { return done(new Error(err)); }
  ...
  describe('Sail-System generate adapter <modelname>', function() {
    var adapterName = 'mongo';

    it('should throw an error if no adapter name is specified', function(done) {

      exec('node ' + Sail-SystemBin + ' generate adapter', function(err) {
        assert.equal(err.code, 1);
        done();
      });
    });
    ...
    it('should create a adapter file in adapters folder', function(done) {
      ...
      exec('node ' + Sail-SystemBin + ' generate adapter ' + adapterName, function(err) {
  ...
  describe('Sail-System generate', function() {
    var modelName = 'post';

    it('should display usage if no generator type is specified', function(done) {

      exec('node ' + Sail-SystemBin + ' generate', function(err, msg) {
        if (err) { return done(err); }
  ...
  describe('Sail-System generate api <apiname>', function() {

    var apiName = 'foo';

    it('should display usage if no api name is specified', function(done) {

      exec('node ' + Sail-SystemBin + ' generate api', function(err, dumb, response) {
    ...
    it('should create a controller and a model file', function(done) {
      ...
      exec('node ' + Sail-SystemBin + ' generate api ' + apiName, function(err) {
        if (err) { return done(err); }
