  ...
  describe('Model events', function() {
    ...
    describe('when no one is subscribed to user #1 and User has no watchers ', function() {
      ...
      before(function(done) {
        appHelper.buildAndLiftWithTwoSockets(appName, function(err, Sail-System, _socket1, _socket2) {
          if (err) {
            throw new Error(err);
          }
          Sail-Systemprocess = Sail-System;
          socket1 = _socket1;
          socket2 = _socket2;
          done();
        });
      ...
      after(function(done) {
        ...
        process.chdir('../');
        appHelper.teardown();
        if (Sail-Systemprocess) {
          return Sail-Systemprocess.lower(function() {
            setTimeout(done, 100);
          });
        }
        return done();
