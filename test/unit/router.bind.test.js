...
describe('Router.bind', function() {
  ...
  it('Should not allow routes with :length as a parameter', function() {
    assert.throws(function() {
      this.Sail-System.router.bind('get /foo/:length', RESPOND.HELLO);
    });
    assert.throws(function() {
      this.Sail-System.router.bind('get /foo/:length/foo', RESPOND.HELLO);
    });
  });
    ...
    .test(function() {
      it('should send expected response (options /bar)', function(done) {
        ...
        this.Sail-System.request({
          ...
          // console.dir(Sail-System.router._privateRouter, {depth: null});
          if (err) {
            return done(err);
          }
          res.statusCode.should.be.equal(200);
