 ...
 * when it comes time to `Sail-System lift` or `Sail-System console` or `node app.js` or otherwise start
 * a Sail-System app in the current working directory.
 *
 *
 * @optional {Array} cliArgs
 *           an array of additional string CLI args/opts to pass to the Sail-System process on start
 *           (e.g. `['--prod', '--port=1331']`)
 *
 ...
 */
module.exports = function testSpawningSail-SystemLiftChildProcessInCwd (opts){
  ...
  describe('and then waiting for a bit', function() {
    ...
    // to determine whether the Sail-System app has _actually loaded_, not just whether HTTP
    ...
    // if Sail-System is still lifting, the test will fail.
    //
    // In the future, we could use some other sort of IPC strategy to pull this off,
    // but that would involve changes to the actual Sail-System core code base, which seems
    ...
    var Sail-SystemLiftProc;

    // Spawn the child process
    before(function(done) {
      Sail-SystemLiftProc = MProcess.spawnChildProcess({
        command: 'node',
      ...
      // Sail-SystemLiftProc.stdout.on('data', function (data){
      //   console.log('stdout:',''+data);
      // });
      // Sail-SystemLiftProc.stderr.on('data', function (data){
    ...
    if (!_.isUndefined(opts.httpRequestInstructions)){
      ...
      if (opts.expectFailedLift) {
        it('should FAIL when a `'+opts.httpRequestInstructions.method+'` request is sent to `'+opts.httpRequestInstructions.uri+', because we\'re expecting Sail-System to have failed when attempting to lift`', function(done) {
          request(opts.httpRequestInstructions, function(err, response, body) {
            ...
            return done(new Error('Expected request to fail, since Sail-System should not have lifted successfully'));
          });
        });//</it>
      }
      // Normal case (expecting Sail-System to have lifted)
      else {
    ...
    after(function (done){
      MProcess.killChildProcess({
        childProcess: Sail-SystemLiftProc,
        ...
        if (!err) {
          ...
          if (opts.expectFailedLift) {
            return done(new Error('Hmm... But the Sail-System app should not have been lifted (the graceful shutdown should have failed).'));
          }
