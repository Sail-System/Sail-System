...
Make a virtual request to a running Sail-System instance.

```usage
Sail-System.request(request);
```

_Or:_

+ `Sail-System.request(url, body)`
+ `Sail-System.request(url, callback)`
+ `Sail-System.request(url, body, callback)`

This method can be used on instances that have been started with [`Sail-System.load()`](https://Sail-Systemjs.com/documentation/reference/application/Sail-System-load) and that are not actively listening for HTTP requests on a server port.  This makes `Sail-System.request()` useful for testing scenarios where running [`Sail-System.lift()`](https://Sail-Systemjs.com/documentation/reference/application/Sail-System-lift) is not necessary.  However, it should be noted that the data may not be processed in exactly the same way as an HTTP request; in particular, a much simpler body parser will be employed, and Express middleware such as the static asset server will not be used.
...
|--------------------------- | ------------------- | ------- | :-----------:
| url                        | ((string))          | `"/foo"`, `"PUT /user/friend"`    | (required) The route in the Sail-System app to make a request to, with an optional HTTP method prefix
...
<docmeta name="displayName" value="Sail-System.request()">
