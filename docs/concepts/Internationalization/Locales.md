...
The i18n hook reads JSON-formatted translation files from your project's "locales" directory (`config/locales` by default).  Each file corresponds with a [locale](http://en.wikipedia.org/wiki/Locale) (usually a language) that your Sail-System backend will support.
...
To override the auto-detected language/localization preference for a request, use [`req.setLocale()`](https://Sail-Systemjs.com/documentation/reference/request-req/req-set-locale), calling it with the unique code for the new locale, e.g.:
