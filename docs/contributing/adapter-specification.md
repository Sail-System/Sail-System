...
+ automatic socket.io pubsub support is provided by Sail-System-- it manages "rooms" for every class (collection) and each instance (model)
...
> All officially supported Sail-System.js database adapters implement the `Semantic` interface.
...
1. emit a declaratively configurable event on the `Sail-System` object.
2. send an HTTP request to a declaratively configurable endpoint.
3. call a function which is part of their declarative config, leveraging the generic `req/res` interpreter in Sail-System
...
They should call Sail-System' `Model.publishUpdate()`, `Model.publishCreate()`, and `Model.publishDestroy()` to publish changes and take advantage of automatic room management functionality.
