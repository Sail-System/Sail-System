 ...
 * Node REPL except that it starts with the Sail-System app in the
 ...
 *
 * @stability 3
 * @see http://Sail-Systemjs.com/documentation/reference/command-line-interface/Sail-System-console
 * ------------------------------------------------------------------------
 * This lifts the Sail-System app in the current working directory, then uses
 * the core `repl` package to spin up an interactive console.
 *
 * Note that, if `--dontLift` was set, then `Sail-System.load()` will be used
 * instead. (By default, the `Sail-System console` cmd runs `Sail-System.lift()`.)
 * ------------------------------------------------------------------------
 */

module.exports = function() {

  // Get a temporary logger just for use in `Sail-System console`.
  ...
  if (configOverrides.dontLift) {
    ...
    cliLogger.info(chalk.gray('Sail-System is not listening for requests (since `dontLift` was enabled).'));
    cliLogger.info(chalk.gray('You still have access to your models, helpers, and `Sail-System`.'));
  }
  else {
  ...
  (function _loadOrLift(proceed){
    ...
    cliLogger.info('Welcome to the Sail-System console.');
    ...
    var repl = REPL.start({
      // Set the REPL prompt.
      prompt: 'Sail-System> ',
      // Allow the REPL to use the same global space as the Sail-System app, giving it access
    ...
    // in the Node REPL for Node 6+, which conflicts with the global _ in Sail-System apps.
    // Note that in Node 6+ this typically causes the REPL to output "Expression assignment to _ now disabled.",
    // which unfortunately gets put right next to the `Sail-System>` prompt, making for a very confusing
    // introduction to the Sail-System console.  In the custom output stream we defined above, we filter out
    // that message.
    if (typeof underscore !== 'undefined') {
      global._ = underscore;
    }
    ...
    var pathToReplHistoryFile = nodepath.join(Sail-SystemApp.config.paths.tmp, '.node_history');
    try {
      ...
      cliLogger.verbose('(This session of `Sail-System console` will still work, it just won\'t support REPL history.)');
