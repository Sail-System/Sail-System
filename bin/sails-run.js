 ...
 * `Sail-System run`
 *
 * Run a script for the Sail-System app in the current working directory.
 *
 ...
 *
 * @see https://Sail-Systemjs.com/documentation/reference/command-line-interface/Sail-System-run
 */
...
module.exports = function(scriptName) {
  ...
  if (!scriptName) {
    ...
    console.error('    Sail-System run rebuild-cloud-sdk');
    console.error();
    console.error('^ runs `scripts/rebuild-cloud-sdk.js`.');
    console.error();
    console.error('(For more help, visit '+chalk.underline('https://Sail-Systemjs.com/support')+'.)');
    return process.exit(1);
  }

  // Remove `scripts/` prefix if it exists (allows users to do Sail-System run scripts/foo, so they can use tab autocomplete).
  scriptName = _.trim(scriptName);
  scriptName = scriptName.replace(/^scripts\//, '');

  // Unless the script name is under a "scope" (as in `@Sail-Systemhq/some-package`), don't allow slashes in the name.
  ...
  // (either a terminal command or a specially-formatted Node.js/Sail-System.js module)
  ...
  try {
    ...
    try {
      ...
      switch (e.code) {
        case 'MODULE_NOT_FOUND': throw flaverr('E_NO_PACKAGE_JSON', new Error('No package.json file.  Are you sure you\'re in the root directory of a Node.js/Sail-System.js app?'));
        default: throw e;
      }
    }
    ...
    switch (e.code) {
      ...
      default:
        ...
        console.error('(You can report suspected bugs at '+chalk.underline('http://Sail-Systemjs.com/bugs')+'.)');
        return process.exit(1);
    }
  }
  ...
  var relativePathToInstalledScript = (function(){
    ...
    installedScriptName = installedScriptName.replace(/^Sail-System-run-/,'');
    return 'node_modules/' + org + 'Sail-System-run-'+installedScriptName;
  })();
  ...
  // If this is a Node.js/Sail-System.js script (machine def), then require the script file
  ...
  if (!pjCommandToRun) {
    try {
      ...
      try {
        ...
        switch (e.code) {
          case 'MODULE_NOT_FOUND': throw flaverr('E_FAILED_TO_REQUIRE_SCRIPT_DEF', new Error('Encountered an error while loading the script definition.  Are you sure this is a well-formed Node.js/Sail-System.js script definition?  Error details:\n'+e.stack));
          default: throw e;
        }
      }
      ...
      if (!_.isObject(scriptDef) || _.isArray(scriptDef) || _.isEqual(scriptDef, {})) {
        ...
        console.error('');
        console.error('A well-formed Node.js/Sail-System.js script should export a script definition.');
        ...
        console.error('        // ...');
        console.error('        Sail-System.log(\'Hello world!\');');
        console.error('        return exits.success();');
        console.error('      }');
        console.error('    };');
        console.error('    ```````````````````````````````````````````````````````````');
        console.error('');
        console.error(' [?] Visit https://Sail-Systemjs.com/support for assistance.');
        console.error('');
        return process.exit(1);
      }

      // Modify the script definition to add `Sail-System: require('Sail-System')` and `habitat: 'Sail-System'`
      // (unless it explicitly disables this behavior with `Sail-System: false` or by explicitly
      // declaring some other habitat)
      var isLifecycleMgmtExplicitlyDisabled = (
        scriptDef.Sail-System === false ||
        (scriptDef.habitat !== undefined && scriptDef.habitat !== 'Sail-System')
      );
      if (!isLifecycleMgmtExplicitlyDisabled) {
        // (Only require the rest of the Sail-System framework if it's needed.)
        var Sail-System = require('../lib/app');
        scriptDef.habitat = 'Sail-System';
        scriptDef.Sail-System = Sail-System();
      }
      ...
      // (we rely on commander having parsed them for us so that we don't include `Sail-System`, `run`, `node`, etc)
  ...
  // Otherwise, this is an NPM script of some kind, from the package.json file.
    ...
    //     console.error('Please resolve any issues and try `Sail-System run '+scriptName+'` again.');
    ...
    (function (proceed){
      ...
      childProcess.on('close', function (code, signal) {
        ...
        // If a signal was received, terminate the current parent process (i.e. `Sail-System run`).
        if (signal) {
