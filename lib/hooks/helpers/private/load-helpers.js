 ...
 * them to the `Sail-System.helpers` dictionary.
 *
 * @param {Sail-SystemApp} Sail-System
 * @param {Function} done
 *        @param {Error?} err
 */
module.exports = function loadHelpers(Sail-System, done) {

  // Load helper defs out of the specified folder
  includeAll.optional({
    dirname: Sail-System.config.paths.helpers,
    filter: /^([^.]+)\.(?:(?!md|txt).)+$/,
    flatten: true,
    ...
    // If any helpers were specified when loading Sail-System, add those on
    // top of the ones loaded from disk.  (Experimental)
    if (Sail-System.config.helpers.moduleDefinitions) {
      // Note that this is a shallow merge!
      _.extend(helperDefs, Sail-System.config.helpers.moduleDefinitions);
    }
    ...
    try {
      ...
      _.each(helperDefs, function(helperDef, identity) {
        try {
          ...
          // Save _fromLocalSail-SystemApp for internal use.
          helperDef._fromLocalSail-SystemApp = true;
          ...
          // Build & expose helper on `Sail-System.helpers`
          // > e.g. Sail-System.helpers.userHelpers.foo.myHelper
          Sail-System.hooks.helpers.furnishHelper(keyPath, helperDef);
        } catch (err) {
      ...
      // errors through the hook callback, which will cause Sail-System to halt lifting.
