...
module.exports = function onRoute (Sail-System, route) {
  ...
  // (This limitation will be improved in a future version of Sail-System.)
  else if (route.target.view.match(/\./)) {
    Sail-System.log.error('Ignoring attempt to bind route (`%s`) to a view with a `.` in the name (`%s`).',route.path, route.target.view);
    return;
  }
  // Otherwise construct an action function which serves a view and then bind it to the route.
    ...
    var referenceInViewsHash = _.get(Sail-System.views, transformedViewAddress);
    ...
    if (referenceInViewsHash === true) {
      return Sail-System.router.bind(route.path, function serveView(req, res) {
    ...
    // known view in the project, so ignore the attempt and inform the user.
      ...
      Sail-System.log.error('Ignoring attempt to bind route (`%s`) to unknown view: `%s`',route.path, route.target.view);
    }
  }
};
