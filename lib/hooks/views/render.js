...
module.exports = function (Sail-System) {
   ...
   * Usage:
   * ```
   * var html = await Sail-System.renderView('emails/reminders/email-verify-your-account', {
  ...
  return function renderView(relPathToView, _options, optionalCb) {
    ...
    return parley(
      function (done) {
        ...
        // here in Sail-System.renderView() as of yet)
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        // > (Note that this IS NO LONGER A DEEP CLONE!  So references to Sail-System config etc
        ...
        // Disable Sail-System built-in layouts for all view engine's except for ejs
        if (Sail-System.config.views.getRenderFn) {
          layout = false;
        }

        var pathToViews = Sail-System.config.paths.views;
        var absPathToView = path.join(pathToViews, relPathToView) + '.' + Sail-System.config.views.extension;
        ...
        _.extend(options, {
          ...
          settings: {
            'view engine': Sail-System.config.views.extension,
            'views': Sail-System.config.paths.views
          }
        });
        ...
        (function _internationalizingIfRelevant(proceed){
          ...
          req.headers['accept-language'] = options.locale || Sail-System.hooks.i18n.defaultLocale;
          req.locale = options.locale || Sail-System.hooks.i18n.defaultLocale;
          Sail-System.hooks.i18n.expressMiddleware(req, options, function(err) {
          ...
          Sail-System.hooks.views._renderFn(absPathToView, options, function(err, compiledHtml){
            if (err) { return done(err); }
