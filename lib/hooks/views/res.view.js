...
module.exports = function _addResViewMethod(req, res, next) {
  ...
  res.guessView = function (locals, couldNotGuessCb) {
    ...
    Sail-System.log.warn(
      '`res.guessView()` is deprecated in Sail-System >= v1.0.  If you want to continue to use it\n'+
      'in your Sail-System app, please just drop its implementation into a hook.\n'+
      ' [?] Unsure or need advice?  Stop by https://Sail-Systemjs.com/support'
    );
  ...
  res.view = function(/* specifiedPath, locals, optionalCb */) {
    ...
    // Sail-System.log.silly('Running res.view() with arguments:',arguments);
    ...
    //  on `req.target` in previous versions of Sail-System)
    ...
    if (_.isFunction(arguments[1])) {
      ...
      if (_.isString(arguments[0])) {
        Sail-System.log.warn('`res.view` called with (path, cb) signature (using path `' + specifiedPath + '`).  You should use `res.view(path, {}, cb)` to render a view without local variables.');
        locals = {};
      }
    }
    ...
    if (Sail-System.config.views.locals) {
      // Formerly a deep merge: `_.merge(locals, Sail-System.config.views.locals, _.defaults);`
      // Now shallow- see https://github.com/balderdashy/Sail-System/issues/3500
      _.defaults(locals, Sail-System.config.views.locals);
    }
    ...
    var pathToViews = Sail-System.config.paths.views;
    ...
    if (!Sail-System.config.views.getRenderFn) {
      ...
      if (locals.layout === undefined || locals.layout === true) {
        layout = Sail-System.config.views.layout;
      }
      ...
      if (layout) {
        ...
        // Sail-System.log.silly('Using layout at: ', absPathToLayout);
      }
    }
    ...
    if (_.isUndefined(res.locals.view)) {
      res.locals.view = {
        ...
        pathFromApp: path.join(path.relative(Sail-System.config.appPath, Sail-System.config.paths.views), relPathToView),
        ext: Sail-System.config.views.extension
      };
    }
    ...
    //
    // Background:
    //  -> https://github.com/balderdashy/Sail-System/pull/3522#issuecomment-174242822
    if (_.isUndefined(res.locals.exposeLocalsToBrowser)) {
      res.locals.exposeLocalsToBrowser = function (options){
        ...
        // non-relevant built-in properties like `Sail-System` and `_`, as well
        ...
        _.each(_.union(_.keys(res.locals), _.keys(locals)), function(localName){
          ...
          // Otherwise, use the one from res.locals... maybe.
            ...
            else if (_.contains(['_', 'Sail-System', 'view', 'session', 'req', 'res', '__dirname', '_layoutFile'], localName)) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      res.locals.__dirname =
        res.locals.__dirname ||
        (absPathToView + '.' + Sail-System.config.views.extension);
    }

    // If silly logging is enabled, display some diagnostic information about the res.view() call:
    if (specifiedPath) { Sail-System.log.silly('View override argument passed to res.view(): ', specifiedPath); }
    Sail-System.log.silly('Serving view at rel path: ', relPathToView);
    Sail-System.log.silly('View root: ', Sail-System.config.paths.views);
    ...
    return res.render(relPathToView, locals, function viewFailedToRender(err, renderedViewStr) {
      ...
      if (typeof optionalCb === 'function') {
        ...
        // if a template error occurred, don't rely on any of the Sail-System request/response methods
        ...
        if (err) {
          ...
          // Sail-System.log.error('Error rendering view at ::', absPathToView);
          // Sail-System.log.error('with layout located at ::', absPathToLayout);
          // Sail-System.log.error(err && err.message);
          //
          //////////////////////////////////////////////////////////////////
        ...
        Sail-System.log.verbose('Rendering view: "%s" (located @ "%s")', relPathToView,absPathToView);
        if (layout) {
          Sail-System.log.verbose('• using configured layout:: %s (located @ "%s")', layout, absPathToLayout);
        }
