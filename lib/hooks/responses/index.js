...
module.exports = function(Sail-System) {
  ...
  return {
    ...
    configure: function() {
      ...
      if (typeof Sail-System.config[500] === 'function') {
        Sail-System.after('lifted', function() {
          STRINGFILE.logDeprecationNotice('Sail-System.config[500]',
            STRINGFILE.get('links.docs.migrationGuide.responses'),
            Sail-System.log.debug);
          Sail-System.log.debug('Sail-System.config[500] (i.e. `config/500.js`) has been superceded in Sail-System v0.10.');
          Sail-System.log.debug('Please define a "response" instead. (i.e. api/responses/serverError.js)');
          Sail-System.log.debug('Your old handler is being ignored. (the format has been upgraded in v0.10)');
          Sail-System.log.debug('If you\'d like to use the default handler, just remove this configuration option.');
        });
      }
      if (typeof Sail-System.config[404] === 'function') {
        Sail-System.after('lifted', function() {
          STRINGFILE.logDeprecationNotice('Sail-System.config[404]',
            STRINGFILE.get('links.docs.migrationGuide.responses'),
            Sail-System.log.debug);
          Sail-System.log.debug('Please define a "response" instead. (i.e. api/responses/notFound.js)');
          Sail-System.log.debug('Your old handler is being ignored. (the format has been upgraded in v0.10)');
          Sail-System.log.debug('If you\'d like to use the default handler, just remove this configuration option.');
        });
      }
    ...
    initialize: function(cb) {
      ...
      // (e.g. {response: 'foo'})
      Sail-System.on('route:typeUnknown', onRoute(Sail-System));
    ...
    loadModules: function(cb) {
      ...
      Sail-System.log.silly('Loading runtime custom response definitions...');
      Sail-System.modules.loadResponses(function loadedRuntimeErrorModules(err, responseDefs) {
    ...
    routes: {
      before: {
        ...
        'all /*': function addResponseMethods(req, res, next) {
          ...
          _.each(Sail-System.middleware.responses, function eachMethod(responseFn, name) {
...
function _mixinJsonxMethod(req, res) {
  ...
  res.jsonx = res.jsonx || function jsonx (data){
    ...
    // Get easy access to Sail-System app instance.
    var Sail-System = req._Sail-System;

    // Log a deprecation notice.
    Sail-System.log.debug('***********************************************************************************');
    Sail-System.log.debug('`res.jsonx()` is deprecated in Sail-System v1.0 and will be removed in a future release.');
    Sail-System.log.debug(chalk.bold('Any files in `api/responses/` that haven\'t been customized can simply be removed.'));
    Sail-System.log.debug(chalk.gray('Otherwise see http://Sail-Systemjs.com/upgrading for options to replace `res.jsonx()`.'));
    if (caller) {
      Sail-System.log.debug(chalk.gray('(jsonx was called from `' + caller + '`)'));
    }
    Sail-System.log.debug('***********************************************************************************');
...
//
// See the Sail-System project roadmap (Sail-Systemjs.com/roadmap) for future plans to better account for this.
