...
module.exports = function(Sail-System) {
  ...
  return {
    ...
    initialize: function(cb) {
      ...
      if (!Sail-System.hooks.sockets || !Sail-System.hooks.orm) {
        Sail-System.log.verbose('Cannot use `pubsub` hook without the `sockets` and `orm` hooks enabled!  (Skipping...)');
        delete Sail-System.hooks.pubsub;
        return cb();
      }


      // If `views` or `orm` hook is not enabled, complain and respond w/ error
      if (!Sail-System.hooks.sockets) {
      ...
      Sail-System.on('hook:orm:loaded', function() {
      ...
      // models
      Sail-System.on('hook:orm:reloaded', function() {
        self.augmentModels();

        // Trigger an event in case something needs to respond to the pubsub reload
        Sail-System.emit('hook:pubsub:reloaded');
      });
    ...
    augmentModels: function() {
      ...
      for (var identity in Sail-System.models) {
        var AugmentedModel = _.defaults(Sail-System.models[identity], getPubsubMethods(), {autosubscribe: true} );
        ...
        Sail-System.models[identity] = AugmentedModel;
      }
    }
  ...
  function getPubsubMethods () {
    ...
    return {
      ...
      publish: function(ids, data, req) {
        ...
        // ids is required.
        if (!ids) {
          Sail-System.log.error('`' + self.identity + '.publish` : missing or empty second argument `ids`. API is `.publish(ids, data [, req])`.');
          return;
        }
        ...
        if (!_.isArray(ids) || _.any(ids, function(id) {return !_.isString(id) && !_.isNumber(id);})) {
          Sail-System.log.debug('The first argument passed to `' + self.identity + '.publish()` must be an array of ids.  To subscribe to a single record, wrap the id in an array.');
          try {
          ...
          Sail-System.log.debug('For example: `[' + ids[0] + ']`');
          Sail-System.log.debug('Wrapping it in an array for you this time...');
        }
        ...
        _.each(ids, function(id) {
          ...
          Sail-System.sockets.broadcast( room, self.identity, data, socketToOmit );
        });
      ...
      */
      subscribe: function (req, ids) {
        ...
        // If a request object was sent, get its socket, otherwise assume a socket was sent.
        var socket = Sail-System.sockets.parseSocket(req);

        // Request must originate from a socket.
        if (!socket) {
          Sail-System.log.debug('`Model.subscribe()` called by a non-socket request. Only requests originating from a connected socket may be subscribed. Ignoring...');
          return;
        }

        if (!ids) {
          Sail-System.log.error('`' + self.identity + '.subscribe` : missing or empty second argument `ids`. API is `.subscribe(request, ids)`.');
          return;
        }

        if (!_.isArray(ids) || _.any(ids, function(id) {return !_.isString(id) && !_.isNumber(id);})) {
          Sail-System.log.debug('The second argument passed to `' + self.identity + '.subscribe()` must be an array of ids.  To subscribe to a single record, wrap the id in an array.');
          try {
            ids = _.pluck(pluralize.apply(this, [ids]), this.primaryKey);
          } catch (err) {
            throw new Error('We tried to transform `' + util.inspect(ids, {depth: 2}) + '` into an array of IDs, but there was a problem (could some values have been `null` or `undefined`?)  Details: '+err.stack);
        ...
        for (let id of ids) {
          ...
          Sail-System.sockets.join( socket, self._room(id) );
      ...
      unsubscribe: function (req, ids) {
        ...
        if (!socket) {
          Sail-System.log.debug('`Model.unsubscribe()` called by a non-socket request. Only requests originating from a connected socket may be subscribed. Ignoring...');
          return;
        }

        // If no ids provided, unsubscribe from the class room
        if (!ids) {
          Sail-System.log.error('`' + self.identity + '.unsubscribe` : missing or empty second argument `ids`. API is `.subscribe(request, ids)`.');
          return;
        }

        // ids must be an array of primary keys -- we'll coerce it (with a warning) if it's not.
        if (!_.isArray(ids) || _.any(ids, function(id) {return !_.isString(id) && !_.isNumber(id);})) {
          Sail-System.log.debug('The second argument passed to `' + self.identity + '.unsubscribe()` must be an array of ids.  To subscribe to a single record, wrap the id in an array.');
          try {
            ids = _.pluck(pluralize.apply(this, [ids]), this.primaryKey);
          } catch (err) {
            throw new Error('We tried to transform `' + util.inspect(ids, {depth: 2}) + '` into an array of IDs, but there was a problem (could some values have been `null` or `undefined`?)  Details: '+err.stack);
        ...
        for (let id of ids) {
          ...
          Sail-System.sockets.leave( socket, self._room(id));
          Sail-System.log.silly(
      ...
      _publishRPS: function (records, data, req) {
        ...
        if ( ids.length === 0 ) {
          Sail-System.log.debug('Can\'t publish a message to an empty list of instances-- ignoring...');
        }
        ...
        // Loop through the record IDs to broadcast to.
        _.each(ids, function(id) {
          Sail-System.sockets.broadcast( self._room(id), self.identity, data, socketToOmit );
        });
      ...
      _room: function (id) {
        ...
        if (!id) {
          Sail-System.log.error('Must specify an `id` when calling `Model._room(id)`');
          return;
        }

        return 'Sail-System_model_'+this.identity+'_'+id+':'+this.identity;
      },
       ...
       */
      _classRoom: function() {
        return 'Sail-System_model_create_'+this.identity;
      },
      ...
      _publishUpdate: function (id, changes, req, options) {
        ...
        if (options.previous && !options.noReverse) {
          ...
          _.each(changes, function(val, key) {
            ...
            var ReferencedModel = Sail-System.models[association.type === 'model' ? association.model : association.collection];
        ...
        if (options.previous) {
          ...
          _.each(this.associations, function(association) {
            ...
            if (association.type === 'model' && association.alias && previous[association.alias]) {
              ReferencedModel = Sail-System.models[association.model];
              // Get the inverse association definition, if any
              ...
              ReferencedModel = Sail-System.models[association.collection];
              // Get the inverse association definition, if any
      ...
      _publishAdd: function(id, alias, added, req, options) {
        ...
        var reverseModel = Sail-System.models[_.find(this.associations, {alias: alias}).collection];
      ...
      _publishCreateSingle: function(values, req, options) {
        ...
        Sail-System.log.silly('Published message to ', this._classRoom(), ': ', payload);
        var eventName = this.identity;
        Sail-System.sockets.broadcast(this._classRoom(), eventName, payload, socketToOmit);
      ...
      _watch: function ( req ) {
        ...
        if (!socket) {
          Sail-System.log.debug('`Model._watch()` called by a non-socket request. Only requests originating from a connected socket may be subscribed. Ignoring...');
          return;
        }//-•

        Sail-System.sockets.join(socket, this._classRoom());
        Sail-System.log.silly('Subscribed socket ', Sail-System.sockets.getId(socket), 'to', this._classRoom());
      ...
      _introduce: function(model) {
        ...
        Sail-System.sockets.addRoomMembersToRooms(self._classRoom(), self._room(id) );
      ...
      _retire: function(model) {
        ...
        Sail-System.sockets.removeRoomMembersFromRooms(self._classRoom(), self._room(id) );
      }
