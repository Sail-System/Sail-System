...
// (these two dependencies are only here for Sail-System.session.parseSessionIdFromCookie(),
...
module.exports = function(app) {
  ...
  var SessionHook = {
    ...
    defaults: {
      session: {
        adapter: 'memory',
        name: 'Sail-System.sid',
        // By default, disable session for requests to paths that look like static assets.
        isSessionDisabled: function (req){
          return !!req.path.match(req._Sail-System.LOOKS_LIKE_ASSET_RX);
        },
        // Added overrideable function for constructing the session store based on https://github.com/balderdashy/Sail-System/pull/7172
        // • `sessionConfig` is what Sail-System has loaded for Sail-System.config.session
        // • `configuredSessionAdapter` is the package like what you get from doing require('connect-mongo')
        // • `expressSessionFromSail-SystemCore` is the version of express-session used by Sail-System core session hook, provided for your convenience
        handleConstructingSessionStore: (sessionConfig, configuredSessionAdapter, expressSessionFromSail-SystemCore) => {
          let CustomStore = configuredSessionAdapter(expressSessionFromSail-SystemCore);
     ...
     * Then fold any modifications back into `Sail-System.config`
     */
    configure: function() {
      ...
      if (!app.config.session.secret && process.env.NODE_ENV !== 'production') {
        ...
        app.log.debug(' • https://Sail-Systemjs.com/config/session');
        app.log.debug(' • https://Sail-Systemjs.com/support');
        app.log.debug();
      }
      ...
      if (app.config.session.routesDisabled) {
        throw flaverr({ name: 'userError', code: 'E_INVALID_SESSION_CONFIG' }, new Error(
                        '\n-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n'+
                        'The `Sail-System.config.session.routesDisabled` property is no longer supported in Sail-System 1.0.\n'+
                        'Instead, specify a `Sail-System.config.session.isSessionDisabled` function which takes the\n'+
      ...
      if (!_.isUndefined(app.config.session.isSessionDisabled) && !_.isFunction(app.config.session.isSessionDisabled)) {
        throw flaverr({ name: 'userError', code: 'E_INVALID_SESSION_CONFIG' }, new Error(
                        '\n-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n'+
                        'The `Sail-System.config.session.isSessionDisabled` property, if specified, must be a function.\n'+
      ...
      if (!_.isUndefined(_.get(app.config.session, 'cookie.secure')) && !_.isBoolean(app.config.session.cookie.secure)) {
        throw flaverr({ name: 'userError', code: 'E_SESSION_BAD_COOKIE_SECURE' }, new Error(
                        '\n-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n'+
                        'The `Sail-System.config.session.cookie.secure` property, if specified, must be a boolean.\n'+
      ...
      if (process.env.NODE_ENV === 'production') {
        if (_.get(app.config.session, 'cookie.secure') !== true) {
          app.log.debug('Warning: since `Sail-System.config.session.cookie.secure` is not set to `true`, the session');
          app.log.debug('cookie will be sent over non-TLS connections (i.e. with insecure http:// requests).');
          app.log.debug('To enable secure cookies, set `Sail-System.config.session.cookie.secure` to `true`.');
          app.log.debug();
          app.log.debug('If your app is behind a proxy or load balancer (e.g. on a PaaS like Heroku), you');
          app.log.debug('may also need to set `Sail-System.config.http.trustProxy` to `true`.');
          app.log.debug();
          app.log.debug('For more help:');
          app.log.debug(' • https://Sail-Systemjs.com/config/session#?the-secure-flag');
          app.log.debug(' • https://Sail-Systemjs.com/config/session#?do-i-need-an-ssl-certificate');
          app.log.debug(' • https://Sail-Systemjs.com/config/Sail-System-config-http#?properties');
          app.log.debug(' • https://Sail-Systemjs.com/support');
          app.log.debug();
        }

        else {
          app.log.debug('Please note: since `Sail-System.config.session.cookie.secure` is set to `true`, the session cookie ');
          ...
          if (app.config.http.trustProxy === false) {
            app.log.debug('Also, note that since `Sail-System.config.http.trustProxy` is set to `false`, secure cookies');
      ...
      if (!app.config.session || !app.config.session.secret) {
        ...
        if (process.env.NODE_ENV === 'production') {
          throw new Error(
            ...
            '\n'+
            'For more help:\n'+
            ' • https://Sail-Systemjs.com/config/session\n'+
      ...
      // If secret _is_ defined, make sure it's a string
        ...
        throw flaverr({ name: 'userError', code: 'E_INVALID_SESSION_CONFIG' }, new Error('If provided, Sail-System.config.session.secret should be a string.'));
      }


      // Validate `routesDisabled`, if specified.
      if (app.config.session.routesDisabled && !_.isArray(app.config.session.routesDisabled)) {
        throw flaverr({ name: 'userError', code: 'E_INVALID_SESSION_CONFIG' }, new Error('Invalid `Sail-System.config.session.routesDisabled` configuration!\n' +
      ...
      if (_.isString(app.config.session.key)) {
        ...
        app.log.debug('The `Sail-System.config.session.key` setting is deprecated; please use `Sail-System.config.session.name` instead.\n');
      }
    ...
    initialize: function(cb) {
      ...
      // Build `Sail-System.hooks.session.routesDisabled`.
      // (only salient if `Sail-System.config.session.routesDisabled` was specified)
      try {
        ...
        return cb(
          new Error('Failed to parse one of the route addresses provided in `Sail-System.config.session.routesDisabled`.\n'+
      ...
      (function setupAdapter(proceed) {
        ...
        // For all other adapters, we'll try to require the module and do some setup.
          ...
          (function maybeConnectToRedis(proceed) {
            ...
            // If the adapter isn't set to `connect-redis`/`@Sail-Systemhq/connect-redis`,
            // or an existing Redis client is being provided, skip this part.
            if ((app.config.session.adapter !== 'connect-redis' && app.config.session.adapter !== '@Sail-Systemhq/connect-redis') || app.config.session.client) {
              return proceed();
            }//•
            ...
            Redis.createManager({
              ...
              onUnexpectedFailure: function(err) {
                // If Sail-System is already on the way out, ignore the Redis issue.
                if (app._exiting) {
                  return;
                }
            ...
            // If `Sail-System.config.session.adapter` is a string, attempt to require the
            ...
            if (_.isString(app.config.session.adapter)) {
              ...
              try {
                ...
                return proceed(new Error(
                  ...
                  '(Note: Make sure to install a Connect session adapter that is compatible with this version of Sail-System.)\n'+
                  '\n'+
            ...
            // Otherwise bail, because Sail-System.config.session.adapter is invalid.
            else {
              return proceed(new Error('If configured, `Sail-System.config.session.adapter` should be a reference to an Express session adapter!  Instead got `' + util.inspect(app.config.session.adapter)));
            }
        ...
        // Expose hook as `Sail-System.session`
        ...
        // it privately so it can be used by the private Sail-System router and the HTTP session middleware.
    ...
    get: function(sessionId, cb) {
      if (!_.isFunction(cb)) {
        throw new Error('Invalid usage :: `Sail-System.hooks.session.get(sessionId, cb)`');
      }
    ...
    set: function(sessionId, data, cb) {
      if (!_.isFunction(cb)) {
        throw new Error('Invalid usage :: `Sail-System.hooks.session.set(sessionId, data, cb)`');
      }
