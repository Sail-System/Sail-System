module.exports = function(Sail-System) {
  ...
  var csurf = require('@Sail-Systemhq/csurf');
  ...
  return function initializeCsrf() {
    ...
    Sail-System.on('router:after', function() {
      ...
      var sortedRouteAddresses = Sail-System.router.getSortedRouteAddresses();
      ...
      _.each(sortedRouteAddresses, function(address) {
        ...
        var target = Sail-System.router.explicitRoutes[address];
        ...
        if (verb === 'get' || verb === 'head' || verb === 'options') {
          Sail-System.log.debug('Ignoring `csrf: ' + target.csrf + '` setting for route `' + verb + ' ' + path + '`.');
          Sail-System.log.debug('CSRF protection does not apply to GET, HEAD or OPTIONS routes.');
        ...
        if (target.csrf === false && Sail-System.config.security.csrf === true) {
          blacklist.push({method: verb || '', regex: regex});
        }

        else if (target.csrf === true && Sail-System.config.security.csrf === false) {
    ...
    Sail-System.on('router:before', function () {
      ...
      Sail-System.router.bind('ALL /*', function clearCSRFTokenLocal (req, res, next) {
        res.locals._csrf = '';
        return next();
      });
      ...
      // where applicable.
      Sail-System.router.bind('/*', function(req, res, next) {

        // If global CSRF is disabled check the whitelist.
        if (Sail-System.config.security.csrf === false) {
