module.exports = function(Sail-System) {
   ...
   * Module loader
   *
   * Load code files from a Sail-System app into memory; modules like controllers,
  ...
  return {
    ...
    initialize: function(cb) {
      // Expose self as `Sail-System.modules`.
      Sail-System.modules = Sail-System.hooks.moduleloader;
      // |
      ...
      // | having those other modules call the appropriate method on `Sail-System.modules.*()`.
    ...
    configure: function() {
      ...
      Sail-System.config.appPath = Sail-System.config.appPath ? path.resolve(Sail-System.config.appPath) : process.cwd();

      _.extend(Sail-System.config.paths, {

        // Configuration
        //
        // For `userconfig` hook
        config: path.resolve(Sail-System.config.appPath, Sail-System.config.paths.config),

        // Server-Side Code
        //
        // For `controllers` hook
        controllers: path.resolve(Sail-System.config.appPath, Sail-System.config.paths.controllers),
        // For `policies` hook
        policies: path.resolve(Sail-System.config.appPath, Sail-System.config.paths.policies),
        // For `services` hook
        services: path.resolve(Sail-System.config.appPath, Sail-System.config.paths.services),
        // For `orm` hook
        adapters: path.resolve(Sail-System.config.appPath, Sail-System.config.paths.adapters),
        models: path.resolve(Sail-System.config.appPath, Sail-System.config.paths.models),
        // For `userhooks` hook
        hooks: path.resolve(Sail-System.config.appPath, Sail-System.config.paths.hooks),
        // For `blueprints` hook
        blueprints: path.resolve(Sail-System.config.appPath, Sail-System.config.paths.blueprints),
        // For `responses` hook
        responses: path.resolve(Sail-System.config.appPath, Sail-System.config.paths.responses),

        // Server-Side HTML
        //
        // For `views` hook
        views: path.resolve(Sail-System.config.appPath, Sail-System.config.paths.views),
        layout: path.resolve(Sail-System.config.appPath, Sail-System.config.paths.layout)
      });
    },
      ...
      async.auto({
        'config/*': function loadOtherConfigFiles (cb) {
          includeAll.aggregate({
            dirname   : Sail-System.config.paths.config,
        ...
        'config/env/**': ['config/local', function loadEnvConfigFolder (asyncData, cb) {
          // If there's an environment already set in Sail-System.config, then it came from the environment
          ...
          var env = Sail-System.config.environment || asyncData['config/local'].environment || 'development';
          includeAll.aggregate({
            dirname   : path.resolve( Sail-System.config.paths.config, 'env', env ),
            filter    : new RegExp('^(.+)\\.(' + SUPPORTED_FILE_EXTENSIONS_FOR_CONFIG.join('|') + ')$'),
            optional  : true,
            flatten   : true,
            keepDirectoryPath: true,
        ...
        'config/env/*' : ['config/local', function loadEnvConfigFile (asyncData, cb) {
          ...
          var env = Sail-System.config.environment || asyncData['config/local'].environment || 'development';
          includeAll.aggregate({
            dirname   : path.resolve( Sail-System.config.paths.config, 'env' ),
        ...
        var env = Sail-System.config.environment;
    ...
    loadAdapters: function (cb) {
      ...
      includeAll.optional({
        dirname: Sail-System.config.paths.adapters,
        filter: /^(.+Adapter)\..+$/,
        replaceExpr: /Adapter/,
        flatten: true,
        depth: 1
        ...
        fs.readdir(Sail-System.config.paths.adapters, function(err, contents) {
          if (err) {
            if (err.code === 'ENOENT') {
          ...
          try {
            _.each(contents, function(filename) {
              ...
              var absPath = path.join(Sail-System.config.paths.adapters, filename);
    ...
    loadModels: function (cb) {
      ...
      includeAll.optional({
        dirname   : Sail-System.config.paths.models,
        filter    : /^(.+)\.(?:(?!md|txt).)+$/,
        replaceExpr : /^.*\//,
        flatten: true
      }, function(err, models) {
        ...
        includeAll.optional({
          ...
          replaceExpr : /^.*\//,
          flatten: true
        }, bindToSail-System(function(err, supplements) {
          if (err) { return cb(err); }

          if (_.keys(supplements).length > 0) {
            Sail-System.log.debug('The use of `.attributes.json` files is deprecated, and support will be removed in a future release of Sail-System.');
          }
    ...
    loadServices: function (cb) {
      includeAll.optional({
        dirname     : Sail-System.config.paths.services,
        filter      : /^(.+)\.(?:(?!md|txt).)+$/,
        depth     : 1,
        caseSensitive : true
      }, bindToSail-System(cb));
    },
    ...
    statViews: function (cb) {
      includeAll.optional({
        dirname: Sail-System.config.paths.views,
        filter: /^(.+)\.(?:(?!md|txt).)+$/,
        replaceExpr: null,
        dontLoad: true
      }, cb);
    ...
    loadPolicies: function (cb) {
      includeAll.optional({
        dirname: Sail-System.config.paths.policies,
        filter: /^(.+)\.(?:(?!md|txt).)+$/,
        replaceExpr: null,
        flatten: true,
        keepDirectoryPath: true
     ...
     * Load app hooks
     *
     * > Note that, while `Sail-System.config.hooks` is respected here in this
     * > function, the `Sail-System.config.loadHooks` setting in regards to
    ...
    loadUserHooks: function (cb) {
      ...
      var appPackageJson;
      try {
        appPackageJson = require(path.resolve(Sail-System.config.appPath, 'package.json'));
      } catch (unusedErr) {
        appPackageJson = {};
      }
      ...
      async.auto({
        ...
        hooksFolder: function(cb) {
          includeAll.optional({
            dirname: Sail-System.config.paths.hooks,
        ...
        nodeModulesFolder: function(cb) {
          includeAll.optional({
            dirname: path.resolve(Sail-System.config.appPath, 'node_modules'),
            filter: /^(package\.json)$/,
            excludeDirs: /^\./,
            // Look inside namespaced folders e.g. node_modules/@Sail-Systemjs/Sail-System-hook-foo
            depth: 3,
            ...
            modules = (function _flatten(modules, installedHooks, currentPath, level) {
              ...
              Object.keys(modules).forEach(function(identity) {
                ...
                if (identity === 'package.json' && modules[identity] === true) {
                  var filePath = path.resolve(Sail-System.config.appPath, 'node_modules', currentPath, identity);
                  try {
                    // Attempt to load the package.json file
                    var packageJson = require(filePath);
                    // If the module isn't declared as a Sail-System hook, ignore it.
                    if (!packageJson.Sail-System || !packageJson.Sail-System.isHook) {
                      return;
                    }
                    ...
                    if (!_.get(appPackageJson, 'dependencies.' + packageJson.name) && !_.get(appPackageJson, 'devDependencies.' + packageJson.name) && !_.get(appPackageJson, 'optionalDependencies.' + packageJson.name)) {
                      Sail-System.log.debug('Ignoring hook `' + packageJson.name + '` because it isn\'t saved as any kind of dependency in your package.json file.');
                      Sail-System.log.debug('(You could try installing it with `npm install ' + packageJson.name +' --save`.  Or if you aren\'t using this hook,');
                      Sail-System.log.debug('just remove it from the node_modules/ folder and this message will stop appearing.)');
                    ...
                    Sail-System.log.verbose('While searching for installable hooks, found invalid package.json file at `'+filePath+'`.  Details:',e.stack);
                    return;
                  }
                }
        ...
        var hooks = _.reduce(results.hooksFolder, function(memo, module, identity) {
          if (Sail-System.config.hooks[identity] !== false && Sail-System.config.hooks[identity] !== 'false') {
        ...
        try {
          ...
          _.extend(hooks, _.reduce(results.nodeModulesFolder, function(memo, modulePackageJson, identity) {
            ...
            // Any special config for this hook will be under the `Sail-System` key in the package.json file.
            var hookConfig = modulePackageJson.Sail-System;
            ...
            // If an identity was specified in Sail-System.config.installedHooks, use that
            else if (Sail-System.config.installedHooks && Sail-System.config.installedHooks[identity] && Sail-System.config.installedHooks[identity].name) {
              hookName = Sail-System.config.installedHooks[identity].name;
            }
            // Otherwise use the module name, with namespacing and initial "Sail-System-hook-" stripped off if it exists
            else {
              // Strip off any NPM namespacing and/or Sail-System-hook- prefix
              hookName = identity.replace(/^(@.+?[\/\\])?(Sail-System-hook-)?/, '');
            }

            if (Sail-System.config.hooks[hookName] === false || Sail-System.config.hooks[hookName] === 'false') {
              return memo;
            }

            // Allow overriding core hooks
            if (Sail-System.hooks[hookName]) {
              Sail-System.log.verbose('Found hook: `'+hookName+'` in `node_modules/`.  Overriding core hook w/ the same identity...');
            }
            ...
            if (hooks[hookName]) {
              ...
              Sail-System.log.warn(err);
              return memo;
            }

            // Load the hook code
            var hook = require(path.resolve(Sail-System.config.appPath, 'node_modules', identity));

            // Set its config key (defaults to the hook name)
            hook.configKey = (Sail-System.config.installedHooks && Sail-System.config.installedHooks[identity] && Sail-System.config.installedHooks[identity].configKey) || hookName;
          ...
          return bindToSail-System(cb)(null, hooks);
    ...
    loadBlueprints: function (cb) {
      includeAll.optional({
        dirname: Sail-System.config.paths.blueprints,
        filter: /^(.+)\.(?:(?!md|txt).)+$/,
    ...
    loadResponses: function (cb) {
      includeAll.optional({
        dirname: Sail-System.config.paths.responses,
        filter: /^(.+)\.(?:(?!md|txt).)+$/,
        useGlobalIdForKeyName: true
      }, bindToSail-System(cb));
    },
   ...
   */
  function bindToSail-System(cb) {
    return function(err, modules) {
      if (err) {return cb(err);}
      _.each(modules, function(moduleDef) {
        // Add a reference to the Sail-System app that loaded the module
        moduleDef.Sail-System = Sail-System;
        // Bind all methods to the module context
        _.bindAll(moduleDef);
      });
      return cb(undefined, modules);
    };
  }//</bindToSail-System definition (private helper function)>
