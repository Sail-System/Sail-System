 ...
 *
 * http://Sail-Systemjs.com/docs/reference/blueprint-api/populate
 *
 */
...
module.exports = function populate(req, res) {
  ...
  Model
    ...
    .exec(function found(err, matchingRecord) {
      ...
      if (!matchingRecord) {
        Sail-System.log.verbose('In `populate` blueprint action: No parent record found with the specified id (`'+parentPk+'`).');
        return res.notFound();
      }//-•

      if (!matchingRecord[attrName]) {
        Sail-System.log.verbose('In `populate` blueprint action: Specified parent record ('+parentPk+') does not have a `'+attrName+'`.');
        return res.notFound();
      }//-•

      // Subcribe to relevant record(s), if appropriate.
      if (Sail-System.hooks.pubsub && req.isSocket) {
