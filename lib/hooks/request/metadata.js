...
module.exports = function _mixinServerMetadata(req) {
  ...
  // Get reference to `Sail-System` (Sail-System app instance) for convenience.
  var Sail-System = req._Sail-System;

  // FUTURE: bring this back, probably.
  // (but note that it wasn't actually being used anyway as of Sail-System v0.12-- was being overridden below)
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // // Access to server port, if available
  // if (Sail-System.hooks.http) {
  //   var nodeHTTPServer = Sail-System.hooks.http.server;
  ...
  if (req.app && req.app.get('trust proxy')) {
  ...
  else if (Sail-System.hooks.http && Sail-System.config.http.trustProxy) {
    ...
    // if the Sail-System.config.http.trustProxy was enabled.
    trustProxy = Sail-System.config.http.trustProxy;
  }
  else {
    trustProxy = false;
  }
