...
module.exports = function(Sail-System) {
  ...
  var policyHookDef = {
    ...
    initialize: function(cb) {
      ...
      self.loadMiddleware(function (err) {
        ...
        Sail-System.log.silly('Finished loading policy middleware functions.  Preparing to bind policies based on config...');
        try {
          self.bindPolicies();
        } catch (e) { return cb(e); }
    ...
    loadMiddleware: function(cb) {
      ...
      Sail-System.log.silly('Loading policy modules from app...');
      Sail-System.modules.loadPolicies(function modulesLoaded (err, modules) {
        if (err) { return cb(err); }

        // Add the loaded policies to our internal dictionary.
        _.extend(self.middleware, modules);

        // If any policies were specified when loading Sail-System, add those on
        // top of the ones loaded from disk.
        if (Sail-System.config.policies && Sail-System.config.policies.moduleDefinitions) {
          _.extend(self.middleware, Sail-System.config.policies.moduleDefinitions);
        }
        ...
        try {
          _.each(_.keys(self.middleware), function(policyName) {
            ...
            if (!_.isFunction(Sail-System.hooks.policies.middleware[policyName])) {
              throw flaverr({ name: 'userError', code: 'E_INVALID_POLICY' }, new Error('Failed loading invalid policy `' + policyName + '` (expected a function, but got a `' + typeof(Sail-System.hooks.policies.middleware[policyName]) + '`)' ));
            }
          });
        } catch (e) {
          return cb(e);
    ...
    bindPolicies: function() {
      ...
      _.each(this.mapping, function(policies, targets) {
        Sail-System.registerActionMiddleware(policies, targets);
      });

      // Emit event to let other hooks know we're ready to go
      Sail-System.log.silly('Policy-controller bindings complete!');
      Sail-System.emit('hook:policies:bound');
    },


    /**
     * Build normalized, hook-internal representation of policy mapping
     * by performing a non-destructive parse of `Sail-System.config.policies`
     *
    ...
    buildPolicyMap: function () {
      ...
      _.each(_.without(_.keys(Sail-System.config.policies), 'moduleDefinitions'), function(key) {

        // Is this a plain dictionary, e.g. UserController: { '*': true } ?
        if (_.isPlainObject(Sail-System.config.policies[key])) {
          ...
          _.each(_.keys(Sail-System.config.policies[key]), function(action) {

            // Get the policies to attach to this action.
            var policies = Sail-System.config.policies[key][action];

            // Add the target/policies mapping to Sail-System.config.policies
            Sail-System.config.policies[controller + '/' + action.toLowerCase()] = policies;
          });

          // Remove the deprecated config key.
          delete Sail-System.config.policies[key];
        ...
        // Make sure all standalone action glob keys are lowercased.
          ...
          Sail-System.config.policies[key.toLowerCase()] = Sail-System.config.policies[key];
          delete Sail-System.config.policies[key];
      ...
      var actionsToProtect = _.without(_.keys(Sail-System.config.policies), 'moduleDefinitions').sort();
      ...
      var mapping = _.reduce(actionsToProtect, function (memo, target, index) {
        ...
        if (Sail-System.config.policies[target] === true || Sail-System.config.policies[target] === false) {
          Sail-System.config.policies[target] = [Sail-System.config.policies[target]];
        }

        // Make sure policies are contained in an array.
        if (!_.isArray(Sail-System.config.policies[target])) {
          Sail-System.config.policies[target] = [Sail-System.config.policies[target]];
        }

        // Get the policies the user wants to add to this set of actions.
        // Note the use of _.compact to transform [undefined] into [].
        var policies = _.compact(_.map(Sail-System.config.policies[target], function(policy) {
          // If the policy is `true`, make sure it's the only one for this target.
          if (policy === true) {
            if (Sail-System.config.policies[target].length > 1) {
          ...
          if (_.isString(policy)) {
            if (!Sail-System.hooks.policies.middleware[policy.toLowerCase()]) {
            ...
            return Sail-System.hooks.policies.middleware[policy.toLowerCase()];
          }
   ...
   */
  Sail-System.on('route:typeUnknown', function bindDirectlyToRoute (event) {
    ...
    if (!Sail-System.hooks.policies.middleware[policyId] ) {
      var routeAddrToDisplay = event.verb ? event.verb+' '+event.path : event.path;
      Err.fatal.__UnknownPolicy__ (policyId, routeAddrToDisplay, Sail-System.config.paths.policies);
    ...
    var fn = Sail-System.hooks.policies.middleware[policyId];
    Sail-System.router.bind(event.path, fn, event.verb, _.merge(event.options, event.target));
  });
