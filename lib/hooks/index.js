...
module.exports = function(Sail-System) {
  ...
  return function Hook(definition) {
    ...
    this.load = function(cb) {
      ...
      var routeCallbacks = function(routes) {
        _.each(routes, function(middleware, route) {
          ...
          Sail-System.router.bind(route, middleware);
        });
      };//ƒ

      // Determine if this hook should load based on Sail-System environment & hook config
      if (this.config.envs &&
        this.config.envs.length > 0 &&
        this.config.envs.indexOf(Sail-System.config.environment) === -1) {
        return cb();
      }
     ...
     *       @returns {Dictionary} [default configuration for this hook to be merged into Sail-System.config]
     */
     ...
     * already in `Sail-System.config` at the time this function is called.  Any modifications
     * should be made in place on `Sail-System.config`
     *
