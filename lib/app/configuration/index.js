...
module.exports = function(Sail-System) {
  ...
  function Configuration() {
    ...
    /**
     * Sail-System default configuration
     *
     * @api private
     */
    this.defaults = function defaultConfig(appPath) {
      ...
      if (_.isObject(Sail-System.config.hooks) && Sail-System.config.hooks.userconfig === false ||
         (_.isArray(Sail-System.config.loadHooks) && Sail-System.config.loadHooks.indexOf('userconfig') === -1)
      ) {
        defaultEnv = Sail-System.config.environment || 'development';
      }
      ...
      return {
        ...
        // from `Sail-System.config` in favor of something more flexible / obvious, e.g. the `app` object
        ...
        hooks: _.reduce(DEFAULT_HOOKS, function (memo, hookBundled, hookIdentity) {
          ...
          // if it's a string, then the core hook is an NPM dependency of Sail-System,
          // so require it (which grabs it from `node_modules/`)
            ...
            try {
              ...
              throw new Error('Sail-System internal error: Could not require(\''+hookBundled+'\').');
            }
            memo[hookIdentity] = hook;
          }
          // otherwise freak out
          else {
            throw new Error('Sail-System internal error: `'+hookIdentity+'`, a core hook, is invalid!');
          }
          return memo;
        }, {}) || {},

        // Save appPath in implicit defaults
        // appPath is passed from above in case `Sail-System lift` was used
        // This is the directory where this Sail-System process is being initiated from.
    ...
    this.load = require('./load')(Sail-System);
