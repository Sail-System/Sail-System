 ...
 * > the order of this array then loads Sail-System over and over.)
 */

module.exports = {
  'moduleloader': true,//<< FUTURE: absorb into core (i.e. federate its methods out to the places where they are used and remove support for `Sail-System.modules`)
  ...
  // -•-  For posterity, this is where the `orm` hook was formerly inserted (please don't get rid of this until we're a few patch releases into Sail-System v1, just so it's easier to reference.)
  ...
  // -•-  For posterity, this is where the `sockets` hook was formerly inserted (please don't get rid of this until we're a few patch releases into Sail-System v1, just so it's easier to reference.)

  'pubsub': true,//<< FUTURE: **pull the private methods into the blueprints hook, and pull the PUBLIC methods into Sail-System-hook-sockets -- i.e. if orm hook available, then Sail-System-hook-sockets decorates models with RPS methods**
  'policies': true,
  'services': true,
  'security': true,
  ...
  // -•-  For posterity, this is where the `grunt` hook was formerly inserted (please don't get rid of this until we're a few patch releases into Sail-System v1, just so it's easier to reference.)
