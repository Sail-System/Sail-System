...
/**
 * Sail-System.prototype.reloadActions()
 *
...
module.exports = function reloadActions(options, cb) {
  ...
  var hooksToReload = _.difference(_.keys(Sail-System.hooks), hooksToSkip);

  // Clear the actions dictionary.
  Sail-System._actions = {};

  // Reload the actions.
  async.each(hooksToReload, function(hookIdentity, next) {
    if (_.isFunction(Sail-System.hooks[hookIdentity].registerActions)) {
      Sail-System.hooks[hookIdentity].registerActions(next);
    } else {
      return next();
    }
    ...
    loadActionModules(Sail-System, cb);
  });
