...
module.exports = function(Sail-System) {
  ...
  return function(results, cb) {
    ...
    if (Sail-System.config.hooks.grunt === false) {
      return cb();
    }
    ...
    // If this app has Sail-System-hook-grunt installed, then we're all good.
    if (
      (appPackageJSON.dependencies && appPackageJSON.dependencies['Sail-System-hook-grunt']) ||
      (appPackageJSON.devDependencies && appPackageJSON.devDependencies['Sail-System-hook-grunt'])
    ) {
      return cb();
    }
    ...
    if (_.contains(GRUNT_FILE_HASHES, hash)) {
      Sail-System.log.debug('Warning: Grunt functionality may not work properly with your current configuration.');
      Sail-System.log.debug('Run `npm install Sail-System-hook-grunt --save` to continue using Grunt with your app.');
