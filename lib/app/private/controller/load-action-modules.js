 ...
 * @param  {Function} cb
 */
module.exports = function loadActionModules (Sail-System, cb) {

  Sail-System.config.paths = Sail-System.config.paths || {};
  Sail-System.config.paths.controllers = Sail-System.config.paths.controllers || 'api/controllers';
  ...
  includeAll.optional({
    dirname: Sail-System.config.paths.controllers,
    filter: /(^[^.]+\.(?:(?!md|txt).)+$)/,
    flatten: true,
    ...
    try {
      ...
      _.each(files, function(moduleDef) {
        ...
        if (match) {
          ...
          _.each(moduleDef, function(action, actionName) {
            ...
            if (actionName === '_config') {
              if (Sail-System.config.blueprints) {
                Sail-System.config.blueprints._controllers[identity.toLowerCase()] = action;
              }
              return;
            }
            ...
            try {
              helpRegisterAction(Sail-System, action, actionIdentity, true);
            } catch (e) {
              switch (e.code) {
        ...
        // a function as its `fn` property.
          ...
          try {
            helpRegisterAction(Sail-System, moduleDef, actionIdentity, true);
          }
          catch (e) {
            switch (e.code) {
      ...
      if (garbage.length) {
        ...
        Sail-System.log.warn('Files in the `controllers` directory may be traditional controllers or \n' +
        ...
        _.each(garbage, function(filePath){Sail-System.log.warn('- '+filePath);});
        Sail-System.log.warn('----------------------------------------------------------------------------\n');
      }

      // (Shallow) merge stuff from Sail-System.config.controllers.moduleDefinitions on top of any loaded files.
      ...
      _.each(_.get(Sail-System, 'config.controllers.moduleDefinitions') || {}, function(action, actionIdentity) {
        helpRegisterAction(Sail-System, action, actionIdentity, true);
      });
    ...
    var actionIdentities = _.keys(Sail-System._actions);
    ...
    _.each(Sail-System._actionMiddleware, function(fns, target) {
