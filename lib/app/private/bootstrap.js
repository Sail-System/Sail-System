...
module.exports = function runBootstrap(done) {
  ...
  // > https://github.com/balderdashy/Sail-System/commit/926baaad92dba345db64c2ec9e17d35711dff5a3
  ...
  if (!Sail-System.config.bootstrap) {
    return done();
  }

  Sail-System.log.verbose('Running the setup logic in `Sail-System.config.bootstrap(done)`...');
  ...
  var timeoutMs = Sail-System.config.bootstrapTimeout || 30000;
  var timer = setTimeout(function bootstrapTookTooLong() {
    ...
    ' [?] Read more: https://Sail-Systemjs.com/config/bootstrap');
  }, timeoutMs);
  ...
  (function(proceed){
    try {
      ...
      if (Sail-System.config.implementationSniffingTactic === 'analogOrClassical') {
        ...
        })(Sail-System.config.bootstrap);//†
        seemsToExpectCallback = hasParameters;
      }
      if (Sail-System.config.bootstrap.constructor.name === 'AsyncFunction') {
        var promise;
        if (seemsToExpectCallback) {
          promise = Sail-System.config.bootstrap(proceed);
        } else {
          promise = Sail-System.config.bootstrap(function(unusedErr){
        ...
        if (seemsToExpectCallback) {
          Sail-System.config.bootstrap(proceed);
        } else {
          Sail-System.config.bootstrap(function(unusedErr){
            proceed(new Error('Unexpected attempt to invoke callback.  Since this "bootstrap" function does not appear to expect a callback parameter, this stub callback was provided instead.  Please either explicitly list the callback parameter among the arguments or change this code to no longer use a callback.'));
          });
          return proceed();
        }
    ...
    if (ranBootstrapFn) {
      if (err) {
        Sail-System.log.error('The bootstrap function encountered an error *AFTER* it already ran once!  Details:',err);
      }
      else {
        Sail-System.log.error('The bootstrap function (`Sail-System.config.bootstrap`) signaled that it is finished, but it already ran once!  (*If* it is using a callback, check that the callback is not being called more than once.)');
      }
      return;
    }//-•
    ranBootstrapFn = true;
