  ...
  return function initializeHooks(hooks, cb) {
    ...
    function prepareHook(id) {
      ...
      if (rawHookFn === 'false') {
        ...
        Sail-System.log.debug('The string "false" was configured for `Sail-System.config.hooks[\''+id+'\']`.');
        Sail-System.log.debug('For compatibility\'s sake, automatically changing this to `false` (boolean).');
        Sail-System.log.debug('(Note that this backwards-compatibility check will be removed in a future');
        Sail-System.log.debug('release of Sail-System, so be sure to update this app ASAP.)');
        rawHookFn = false;
      ...
      // ```
      //
      // But it was removed in Sail-System v1, since it was no longer relevant.
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      if (!_.isFunction(rawHookFn)) {
        Sail-System.log.error('Malformed hook! (' + id + ')');
        Sail-System.log.error('Hooks should be a function with one argument (`Sail-System`)');
        Sail-System.log.error('But instead, got:', rawHookFn);
        process.exit(1);
      }

      // Instantiate the hook
      var def = rawHookFn(Sail-System);
    ...
    function applyDefaults(hook) {
      ...
      var defaults = (_.isFunction(hook.defaults) ?
                            hook.defaults(Sail-System.config) :
                            hook.defaults) || {};
      ...
      defaultsDeep(Sail-System.config, defaults);
    }//ƒ
    ...
    function loadHook(id, cb) {
      ...
      if (!_.isUndefined(Sail-System.config.hookTimeout)) {
        if (!_.isNumber(Sail-System.config.hookTimeout) || Sail-System.config.hookTimeout < 1 || Math.floor(Sail-System.config.hookTimeout) !== Sail-System.config.hookTimeout) {
          return cb(new Error('Invalid `hookTimeout` config!  If set, this should be a positive whole number, but instead got `'+Sail-System.config.hookTimeout+'`.  Please change this setting, then try lifting again.'));
        }
      }

      var timestampBeforeLoad = Date.now();
      var DEFAULT_HOOK_TIMEOUT = 40000;
      var timeoutInterval = (Sail-System.config[hooks[id].configKey || id] && Sail-System.config[hooks[id].configKey || id]._hookTimeout) || Sail-System.config.hookTimeout || DEFAULT_HOOK_TIMEOUT;
      ...
      if (id !== 'userhooks') {
        hookTimeout = setTimeout(function tooLong() {
          ...
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          var err = new Error(
            'Sail-System is taking too long to load.\n'+
            '\n'+
            '--  --  --  --  --  --  --  --  --  --  --  --  --  --  --  --  --  --  --  --  --\n'+
            ...
            '    (Try increasing the hook timeout.  Currently it is '+(Sail-System.config.hookTimeout||DEFAULT_HOOK_TIMEOUT)+'.\n'+
            '     e.g. `Sail-System lift --hookTimeout='+(Math.max(DEFAULT_HOOK_TIMEOUT, 2*(Sail-System.config.hookTimeout||DEFAULT_HOOK_TIMEOUT)))+'`)\n'+
            '\n'+
      ...
      hooks[id].load(function(err) {
        ...
        if (err) {
          ...
          if (id !== 'userhooks') {
            Sail-System.log.error('A hook (`' + id + '`) failed to load!');
          }
          Sail-System.emit('hook:' + id + ':error');
        ...
        Sail-System.log.verbose(id, 'hook loaded successfully. ('+(Date.now() - timestampBeforeLoad)+'ms)');
        Sail-System.emit('hook:' + id + ':loaded');
    ...
    async.series(
      {
        ...
        validate: function(cb) {
          if (hooks.controllers) {
            ...
            Sail-System.log.debug('Ignoring `controllers` hook:');
            Sail-System.log.debug('As of Sail-System v1, `controllers` can no longer be disabled/enabled as hooks.');
            Sail-System.log.debug('Instead, Sail-System core now understands controller actions as first-class citizens.');
            Sail-System.log.debug('See the Sail-System v1.0 upgrade guide: http://Sail-Systemjs.com/upgrading');
            Sail-System.log.debug('=================================================================================');
        ...
        load: function(cb) {
          async.each(_.without(_.keys(hooks), 'userconfig', 'moduleloader', 'userhooks'), function (id, cb) {
            Sail-System.log.silly('Loading hook: ' + id);
            loadHook(id, cb);
          }, cb);
        }
      },
