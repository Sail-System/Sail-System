...
module.exports = function _buildResponse (req, _res) {
  ...
  if (res._clientCallback) {
    ...
    // using `express-session` directly as of https://github.com/balderdashy/Sail-System/commit/58e93f5a5f2e667e3fbeddf5b4b356f813e3555e.
  ...
  res.send = res.send || function _sendShim (data, noLongerSupported) {
    if (!_.isUndefined(noLongerSupported)) {
      throw new Error('The 2-ary usage of `res.send()` is no longer supported in Express 4/Sail-System v1.  Please use `res.status(statusCode).send(body)` instead.');
    }
    ...
    try {
      ...
      if (req._Sail-System && req._Sail-System.log && req._Sail-System.log.error) {
        req._Sail-System.log.error(e);
        return;
      }
      console.error(e);
      return;
  ...
  res.json = res.json || function _jsonShim (data, noLongerSupported) {
    if (!_.isUndefined(noLongerSupported)) {
      throw new Error('The 2-ary usage of `res.json()` is no longer supported in Express 4/Sail-System v1.  Please use `res.status(statusCode).json(body)` instead.');
    }
  ...
  res.render = res.render || function _renderShim (relativeViewPath, locals, cb) {
    ...
    try {
      if (!req._Sail-System) {
        throw new Error('Cannot call res.render() - `req._Sail-System` was not attached');
      }
      if (!req._Sail-System.renderView) {
        throw new Error('Cannot call res.render() - `req._Sail-System.renderView` was not attached (perhaps `views` hook is not enabled?)');
      }
      ...
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      // TODO:
      // Instead of this shim, turn `Sail-System.renderView` into something like
      // `Sail-System.hooks.views.render()`, and then call it.
      ...
      if (req._Sail-System && req._Sail-System.log && req._Sail-System.log.error) {
        req._Sail-System.log.error('res.render() failed: ', e);
      }
      else {
  ...
  res.redirect = res.redirect || function _redirectShim (address, noLongerSupported) {
    if (!_.isUndefined(noLongerSupported)) {
      throw new Error('The 2-ary usage of `res.redirect()` is no longer supported in Express 4/Sail-System v1.  Please use `res.status(statusCode).redirect(address)` instead.');
    }
