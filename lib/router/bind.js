...
function bind( /* path, target, verb, options */ ) {
  ...
  if (path.match(/\/:length($|\/)/)) {
    ...
    'Routes which contain `/:length` in their address URL are not supported by Sail-System/Express (consider using `/:len`)'));
  }
  ...
  // of the keys as route options and then bind the target.
    ...
    // accordingly.  This makes it easier to add functionality to Sail-System.
    Sail-System.emit('route:typeUnknown', {
      path: path,
      target: target,
      verb: verb,
      options: options
...
function bindRedirect(path, redirectTo, verb, options) {
  ...
  bind.apply(this,[path, function(req, res) {
    Sail-System.log.verbose('Redirecting request (`' + path + '`) to `' + redirectTo + '`...');
...
function bindAction(path, target, verb, options) {
  ...
  try {
    ...
    throw flaverr({name: e.name || 'Sail-SystemError', code: e.code || 'E_UNKNOWN_BIND_ERROR'}, new Error('Error attempting to bind `' + (verb || 'ALL') + ' ' + path + '` to ' + JSON.stringify(target) + ': ' + e.message));
  }
  ...
  if (!Sail-System._actions[actionIdentity]) {
    Sail-System.log.warn('Ignored attempt to bind route (' + path + ') to unknown action ::', target);
    return;
  }

  // Add "action" property to the route options, and set the _middlewareType property if the function doesn't already have one.
  _.extend(options || {}, {action: actionIdentity, _middlewareType: (Sail-System._actions[actionIdentity] && Sail-System._actions[actionIdentity]._middlewareType || 'ACTION: ' + actionIdentity)});

  // Loop through all of the registered action middleware, and find
  // any that should apply to the action with the given identity.
  var actionMiddlewareToRun = _.reduce(Sail-System._actionMiddleware, function(memo, middlewareList, key) {
  ...
  bind.apply(this,[path, function(req, res, next) {
    ...
    // bail out early.
    if (!_.isFunction(Sail-System._actions[actionIdentity])) {
    ...
    try {
      ...
      if (Sail-System._actions[actionIdentity].constructor.name === 'AsyncFunction') {
        ...
        var promise = Sail-System._actions[actionIdentity](req, res, mockNext);
      ...
      // For synchronous actions, just call the function.
        ...
        // Call the action with the specified identity, passing in req and res, as well as `mockRes` to catch unauthorized
        // use of `next` inside of end-user action code.
        return Sail-System._actions[actionIdentity](req, res, mockNext);
      }
    } catch(e) {
      // If we do catch an error, use `next` to let Express handle it correctly.
      return next(e);
...
function bindArray(path, target, verb, options) {
  ...
  if (target.length === 0) {
    Sail-System.log.verbose('Ignoring empty array in `router.bind(' + path + ')`...');
  } else {
...
function bindFunction(path, fn, verb, options) {
  ...
  if (_.intersection(_.keys(options), ['populate', 'skip', 'limit', 'sort', 'where']).length > 0) {
    Sail-System.log.debug('In route `' + verb + ' ' + path + ':');
    Sail-System.log.debug('The `populate`, `skip`, `limit`, `sort` and `where` route options are no longer supported in Sail-System 1.0.');
    Sail-System.log.debug('Instead, you can use a `parseBlueprintOptions` function to fully customize blueprint behavior for a route.');
  ...
  Sail-System.log.silly('Binding route :: ', verb || '', path, _middlewareType);
  ...
  var skipRegexesWrapper = function(regexes, fn) {
    ...
    regexes = _.compact(regexes.map(function(regex) {
      ...
      Sail-System.log.warn('Invalid regex "' + regex + '" supplied to skipRegexesWrapper; ignoring.');
      return undefined;
    }));
  ...
  if (options.skipAssets) {
    options.skipRegex.push(Sail-System.LOOKS_LIKE_ASSET_RX);
  }
  ...
  targetVerb.forEach(function(verb) {
    ...
    Sail-System.router._privateRouter[verb](path, targetFn);
    ...
    // Sail-System.hooks.http.app[verb || 'all'](path, target);
    Sail-System.emit('router:bind', {
      path: path,
      target: targetFn,
      verb: verb,
      options: options,
...
/**
 * Sanitize the arguments to `Sail-System.router.bind()`
 *
