...
var router = require('@Sail-Systemhq/router');
...
module.exports = function(Sail-System) {
  return new Router({Sail-System: Sail-System});
};
...
function Router(options) {
  ...
  options = options || {};
  this.Sail-System = options.Sail-System;
  this.defaultHandlers = defaultHandlers(this.Sail-System);

  // Expose router on `Sail-System` object
  this.Sail-System.router = this;
...
Router.prototype.load = function(results, cb) {
  var Sail-System = this.Sail-System;

  Sail-System.log.silly('Loading router...');

  // Maintain a reference to the static route config
  this.explicitRoutes = Sail-System.config.routes;

  // Save reference to Sail-System logger
  this.log = Sail-System.log;
  ...
  if (!Sail-System.hooks.session) {
    ...
    if (sessionSecret) {
      ...
      if (!_.isString(sessionSecret)) {
        return cb(new Error('If provided, Sail-System.config.session.secret should be a string.'));
      }
    }
  }
  if (sessionSecret) {
    Sail-System._privateCpMware = require('cookie-parser')(sessionSecret);
  } else {
    Sail-System._privateCpMware = require('cookie-parser')();
  }
  ...
  // Listen for requests
  Sail-System.on('router:request', this.route);

  // Listen for unhandled errors and unmatched routes
  Sail-System.on('router:request:500', this.defaultHandlers[500]);
  Sail-System.on('router:request:404', this.defaultHandlers[404]);
...
/**
 * `Sail-System.router.route(partialReq, partialRes)`
 *
 ...
 * since the true HTTP req/res streams already exist.  In that case, at lift-time, Sail-System
 * calls `router:bind`, which loads Sail-System' routes as normal middleware/routes in the http hook.
 ...
 * for routing to Sail-System' default `next(foo)` handler.
 *
 * @param {Request} req
 * @param {Response} res
 * @api private
...
Router.prototype.route = function(req, res) {
  ...
  // If Sail-System is `_exiting`, ignore the request.
  if (Sail-System._exiting) {
    return;
  }

  // Provide access to Sail-SystemApp instance as `req._Sail-System`.
  req._Sail-System = req._Sail-System || Sail-System;
  ...
  Sail-System.log.silly('Handling virtual request :: Running virtual querystring parser...');
  qsParser(req,res, function (err) {
    ...
    parseCookies(req, res, function(err){
      ...
      loadSession(req, res, function (err) {
        ...
        Sail-System.log.silly('Handling virtual request :: Running virtual body parser...');
        bodyParser(req,res, function (err) {
          ...
          _privateRouter(req, res, function handleUnmatchedNext(err) {
            ...
            if (err) {
              Sail-System.log.silly('Handling virtual request :: Running final "error" handler...');
              Sail-System.emit('router:request:500', err, req, res);
              return;
            }

            // Or the default not found handler
            Sail-System.log.silly('Handling virtual request :: Running final "not found" handler...');
...
Router.prototype.unbind = function(routeToRemove) {
  ...
  Sail-System.emit('router:unbind', routeToRemove);
...
Router.prototype.flush = function(routes) {
  ...
  Sail-System.emit('router:before');
  ...
  _.each(sortedRouteAddresses, function(address) {
    ...
    if (shouldDeclareSkipAssets) {
      Sail-System.log.warn('Warning: route `' + address + '` should explicitly declare `skipAssets: true` or `skipAssets: false` to ensure correct handling of assets!');
      Sail-System.log.warn('See http://Sail-Systemjs.com/docs/concepts/routes/url-slugs for more info.');
      console.log();
    }
  ...
  if (_.isObject(target) && !_.isArray(target) && !_.isFunction(target)) {
    ...
    if (!actionIdentity.match(/^[a-zA-Z_\$]+[a-zA-Z0-9_\/\-\$]*$/)) {
      ...
      throw flaverr({name: 'userError', code: 'E_NOT_ACTION_TARGET'}, new Error(
        ...
        'See http://Sail-Systemjs.com/docs/concepts/routes/custom-routes#?controller-action-target-syntax\n'+
...
function bodyParser (req, res, next) {
  ...
  if ( !(req instanceof Readable) ) {
    return next(new Error('Sail-System Internal Error: `req` should be a Readable stream by the time `route()` is called'));
  }
...
function parseCookies (req, res, next){
  ...
  // req._Sail-System.log.verbose('Parsing cookie:',req.headers.cookie);

  if (req._Sail-System._privateCpMware) {
    // Run the middleware
    return req._Sail-System._privateCpMware(req, res, next);
  }
...
function loadSession (req, res, next){
  ...
  // session middleware.
  if (req._Sail-System._privateSessionMiddleware && !req.headers.nosession) {

    // Access store preconfigured session middleware as a private property on the app instance.
    return req._Sail-System._privateSessionMiddleware(req, res, next);
  }
