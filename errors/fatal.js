...
module.exports = {
  ...
  failedToLoadSail-System: function(err) {
    log.error();

    // If the error is something the user can fix (as opposed to an internal Sail-System error AKA bug),
    // just show the error message, not the whole stack trace into Sail-System core.
    ...
    console.error();
    log.error('Could not load Sail-System app.');
    log.error();
    log.error('Tips:');
    log.error(' • First, take a look at the error message above.');
    log.error(' • Make sure you\'ve installed dependencies with `npm install`.');
    log.error(' • Check that this app was built for a compatible version of Sail-System.');
    log.error(' • Have a question or need help?  (http://Sail-Systemjs.com/support)');
    _terminateProcess(1);
  },

  noPackageJSON: function() {
    log.error('Cannot read package.json in the current directory (' + process.cwd() + ')');
    log.error('Are you sure this is a Sail-System app?');
    _terminateProcess(1);
  },

  notSail-SystemApp: function() {
    log.error('The package.json in the current directory does not list Sail-System as a dependency...');
    log.error('Are you sure `' + process.cwd() + '` is a Sail-System app?');
    _terminateProcess(1);
  },
  ...
  badLocalDependency: function(pathToLocalSail-System, requiredVersion) {
    ...
    log.error('rm -rf ' + pathToLocalSail-System + ' && npm install Sail-System@' + requiredVersion);
    _terminateProcess(1);
  },
  ...
  malformedHook: function() {
    ...
    log.error('Hooks should be a function with one argument (`Sail-System`)');
    _terminateProcess(1);
  },
  ...
  invalidCustomResponse: function(responseIdentity) {
    ...
    log.error('`res.' + responseIdentity + '` has special meaning in Connect/Express/Sail-System.');
  ...
  __UnknownConnection__: function(connectionId, sourceModelId) {
    ...
    log.error('Are you sure that connection exists?  It should be defined in `Sail-System.config.connections`.');

    // var probableAdapterModuleName = connectionId.toLowerCase();
    // if ( ! probableAdapterModuleName.match(/^(Sail-System-|waterline-)/) ) {
    //   probableAdapterModuleName = 'Sail-System-' + probableAdapterModuleName;
    // }
    // log.error('Otherwise, if you\'re trying to use an adapter named `' + connectionId + '`, please run ' +
    //   '`npm install ' + probableAdapterModuleName + '@' + Sail-System.majorVersion + '.' + Sail-System.minorVersion + '.x`');
    return _terminateProcess(1);
  },
  ...
  __UnknownAdapter__: function(adapterId, sourceModelId /*, Sail-SystemMajorV, Sail-SystemMinorV */) {
    log.error('Trying to use unknown adapter, "' + adapterId + '", in model `' + sourceModelId + '`.');
    log.error('Are you sure that adapter is installed in this Sail-System app?');
    ...
    log.error('Otherwise, if you\'re trying to use an adapter named `' + adapterId + '`, please run ' +
      '`npm install ' + probableAdapterModuleName + ' --save'/*'@' + Sail-SystemMajorV + '.' + Sail-SystemMinorV + '.x`'*/);
    return _terminateProcess(1);
  },
  ...
  __InvalidAdapter__: function(attemptedModuleName, supplementalErrMsg) {
    ...
    log.error('Is this a valid Sail-System/Waterline adapter?  The following error was encountered ::');
...
function _terminateProcess(code /*, opts */) {
  ...
  if (process.env.NODE_ENV === 'test') {
    throw new Error({
      ...
      // }
      // ^^ Removed this in Sail-System v1 since it was useless anyways. ~Mike Dec 11, 2016
    });
  }//-•
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
